...
public class DefaultBukkitScheduler implements AdaptedScheduler {
    ...
    public DefaultBukkitTask runTaskLater(@NotNull Runnable runnable, long delay) {
        return new DefaultBukkitTask(this.scheduler.runTaskLater(this.plugin, runnable, Math.max(0L, delay)));
    }

    @Override
    @NotNull
    public DefaultBukkitTask runTaskLaterAsync(@NotNull Runnable runnable, long delay) {
        return new DefaultBukkitTask(this.scheduler.runTaskLaterAsynchronously(this.plugin, runnable, Math.max(0L, delay)));
    }

    @Override
    @NotNull
    public DefaultBukkitTask runTaskTimer(@NotNull Runnable runnable, long delay, long period) {
        return new DefaultBukkitTask(this.scheduler.runTaskTimer(this.plugin, runnable, Math.max(0L, delay), period));
    }

    @Override
    @NotNull
    public DefaultBukkitTask runTaskTimerAsync(@NotNull Runnable runnable, long delay, long period) {
        return new DefaultBukkitTask(this.scheduler.runTaskTimerAsynchronously(this.plugin, runnable, Math.max(0L, delay), period));
    }
}
